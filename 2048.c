...
int menu(WINDOW *wnd, char *variante[], WINDOW *joc, int **mat, WINDOW *scor,
         int *puncte) {
  int opt, i, high = 0;
  while (FOREVER) {
    wclear(wnd);
...
void new_game(WINDOW *joc, WINDOW *scor, WINDOW *stats, int *puncte, int ***mat,
              char com[][15], int *nr_sus, int *nr_jos, int *nr_stanga,
              int *nr_dreapta, int *nr_auto, int *nr_undo, int ****undo,
              int *n) {
...
void resume(WINDOW *joc, WINDOW *scor, WINDOW *stats, int *puncte, int ***mat,
            char com[][15], int *nr_sus, int *nr_jos, int *nr_stanga,
            int *nr_dreapta, int *nr_auto, int *nr_undo, int ****undo, int *n) {
...
int main(void) {
  ...
  while (FOREVER) {
    ...
    case 0:
      ...
      wrefresh(wnd);
      leg(legenda);
      new_game(joc, scor, stats, &puncte, &mat, com, &nr_sus, &nr_jos,
               &nr_stanga, &nr_dreapta, &nr_auto, &nr_undo, &undo, &n);
      refresh();
      break;
    case 1:
      if (mat_gol(mat) == 1 && win(mat) != 1 && win(mat) != 2) {
        ...
        wrefresh(wnd);
        leg(legenda);
        resume(joc, scor, stats, &puncte, &mat, com, &nr_sus, &nr_jos,
               &nr_stanga, &nr_dreapta, &nr_auto, &nr_undo, &undo, &n);
      }
      break;
    default:
